...
<title>Create Account — DevPath</title>
...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  /* FORM CONTAINER */
  .form-box{background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:3rem;max-width:480px;width:100%;z-index:1;animation:fadeup 0.6s ease both;max-height:90vh;overflow-y:auto}
  ...
  /* FORM GROUP */
  .form-group{margin-bottom:1.2rem;animation:fadeup 0.6s ease both}
  .form-label{display:block;font-family:var(--font-mono);font-size:0.8rem;color:var(--muted);margin-bottom:0.4rem;text-transform:uppercase;letter-spacing:0.05em;transition:color 0.2s}
  .form-input{width:100%;background:var(--bg3);border:1px solid var(--border);color:var(--text);padding:0.7rem 0.9rem;border-radius:8px;font-family:var(--font-sans);font-size:0.9rem;transition:border-color 0.3s cubic-bezier(0.4,0,0.2,1),box-shadow 0.3s cubic-bezier(0.4,0,0.2,1),background 0.2s;}
  ...
  /* CHECKBOX */
  .form-checkbox{display:flex;align-items:flex-start;gap:0.6rem;font-family:var(--font-mono);font-size:0.75rem;margin-bottom:1.5rem;animation:fadeup 0.6s 0.4s ease both;line-height:1.5}
  .checkbox-input{width:18px;height:18px;border:1px solid var(--border);border-radius:4px;background:transparent;cursor:pointer;accent-color:var(--blue);margin-top:0.2rem;flex-shrink:0;transition:border-color 0.2s,box-shadow 0.2s}
  ...
  /* DIVIDER */
  .divider{display:flex;align-items:center;gap:1rem;margin:1.5rem 0;animation:fadeup 0.6s 0.6s ease both}
  .divider-line{flex:1;height:1px;background:var(--border)}
  .divider-text{font-family:var(--font-mono);font-size:0.7rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.05em}

  /* OAUTH */
  .oauth-buttons{display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.5rem;animation:fadeup 0.6s 0.7s ease both}
  ...
  /* FOOTER */
  .form-footer{text-align:center;margin-top:1.5rem;animation:fadeup 0.6s 0.8s ease both}
  ...
  .password-strength{display:flex;gap:0.3rem;margin-top:0.5rem;margin-bottom:0.8rem}
  .strength-bar{height:3px;flex:1;background:var(--border);border-radius:2px;transition:background 0.3s ease,transform 0.3s ease}
  .strength-bar.weak{background:var(--red);animation:strengthen 0.4s ease}
  .strength-bar.fair{background:var(--yellow);animation:strengthen 0.4s ease}
  .strength-bar.good{background:var(--blue);animation:strengthen 0.4s ease}
  .strength-bar.strong{background:var(--green);animation:strengthen 0.4s ease}

  /* ANIMATIONS */
  @keyframes fadeup{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
  @keyframes slidedown{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}}
  @keyframes strengthen{from{transform:scaleX(0.5)}to{transform:scaleX(1)}}

  /* SCROLLBAR STYLING */
  .form-box::-webkit-scrollbar{width:6px}
  .form-box::-webkit-scrollbar-track{background:var(--bg3)}
  .form-box::-webkit-scrollbar-thumb{background:var(--border);border-radius:3px}
  .form-box::-webkit-scrollbar-thumb:hover{background:var(--border2)}
</style>
</head>
<body>
...
<section class="auth-container">
  <div class="form-box">
    <div class="form-header">
      <div class="form-tag">Build something real</div>
      <h1>Create your DevPath account</h1>
      <p>Start your journey to shipping real products</p>
    </div>
   ...
   <form id="signupForm" action="register.php" method="POST">
      ...
      <div class="form-group">
        ...
        <input type="password" id="password" name="password" class="form-input" placeholder="••••••••" required onchange="checkPasswordStrength()">
      ...
      <div class="form-group">
        ...
        <select id="role" name="role" class="form-input" required style="background:var(--bg3);cursor:pointer">
          <option value="">Select your role</option>
          <option value="solo">Solo Developer / Learner</option>
      ...
      <div class="form-checkbox">
        ...
        <label for="newsletter">Get updates on new features and tips for shipping real products</label>
      </div>
...
<script>
  function checkPasswordStrength() {
    ...
    strengthBars.forEach((bar, index) => {
      ...
      // Remove animation class before adding new one
      void bar.offsetWidth; // Trigger reflow for animation restart
    });

    const strengthLevels = ['weak', 'fair', 'good', 'strong'];
    for (let i = 0; i < strength; i++) {
      strengthBars[i].classList.add(strengthLevels[strength - 1]);
    }
  }
